...
[jacobfourier,lineint,totalstroke] = evaluate_jacobian_fourier(y,s,n,dimension,direction);
...
if nargout>1
    if strcmpi(s.costfunction,'torque') || strcmpi(s.costfunction,'covariant acceleration') || strcmpi(s.costfunction,'acceleration coord') || strcmpi(s.costfunction,'power quality')
        ...
        % inertia systems
        g = -jacobfourier.total; % Optimizing for displacement over cost
%         g = -jacobfourier.disp; % Optimizing for displacement only
        ...
        g=[-jacobfourier.total;zeros(1,dimension)]; % Optimizing for displacement over cost
